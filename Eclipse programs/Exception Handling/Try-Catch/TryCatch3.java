...
//In this program we have multiple catch blocks in case we don't know what kind of exception might occur. 
	public static void main(String[] args) {
		...
		try {
			...
			//catch block one catching a NullPointerException if occured.
		catch(NullPointerException e) {
			System.out.println("Invalid");
		}
			//catch block two catching a ArrayIndexOutOfBoundsException if occured.
		catch(ArrayIndexOutOfBoundsException e) {
			System.out.println("Invalid index");
		}
			//catch block third will catch all the exceptions as Exeception is the Superclass of all Exceptions. So, it can be used if we don't know the exception name.
